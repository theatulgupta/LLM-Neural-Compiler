...
def test_int8_dynamic_tiny(tiny_path) -> None:
    ...
    quantized = apply_pass(
        "quantize_dynamic_int8", loaded.model, {"per_channel": True, "weight_type": "qint8"}
    )
    ops = graph_ir_snapshot(quantized)["op_counts"]
    assert (
        "ConvInteger" in ops
        or "MatMulInteger" in ops
        or "QLinearConv" in ops
        or "DynamicQuantizeLinear" in ops
    )
